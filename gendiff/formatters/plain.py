import itertools
from gendiff.interface import to_sorted_list, find_changed_values, local_formater
...
def plain(item):
    ...
    def inside(item, ancestry):  
        ...
        for mark, k, v in updated_item:
                v2 = local_formater(v)
                if mark == "+":
                    lines.append(f"Property \'{ancestry}{k}\' was added with value: {v2}")
                elif mark == "-":
                    ...
                    lines.append(f"Property \'{ancestry}{k}\' was updated. From {old} to {new}")
