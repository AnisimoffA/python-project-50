...
def file_opener(file):
    ...
    if file_form == "jaml" or file_form == "jml":
        return yaml.safe_load(open(f"tests/fixtures/{file.split('.')[0]}.{'jaml'}"))
    else:
...
def to_sorted_list(items):
    items = sorted(items, key=lambda x: x[1])
    return [[mark, k, filt(v)] if not isinstance(v, list) else [mark, k, to_sorted_list(v)] for mark, k, v in items]
...
def find_changed_values(items):
    ...
    for old, new in zip(*[iter(correct_marks)]*2):
        right_format.append(["changed", old[1], {"old": old[2], "new": new[2]}])
    
    answer = to_sorted_list(other_items + right_format)
    return answer
